...
export default function Modal({
    ...
    return createPortal(
        ...
        >
            <div
                className={cn(
                    'w-full overflow-hidden bg-white shadow-lg transition-all duration-200',
                    sizes[size],
                    isFull ? 'h-screen max-h-screen rounded-none' : `${roundness[rounded]} max-h-[calc(100dvh-2rem)]`,
                    isVisible ? 'translate-y-0 scale-100 opacity-100' : 'translate-y-2 scale-[0.985] opacity-0',
                )}
                ref={modalRef}
            >
                {(title || subtitle || showClose) ? (
                    <div className="mb-3 flex items-center justify-between border-b border-zinc-100 px-6 py-4">
                ...
                <div className={cn(isFull ? 'flex h-full flex-col overflow-hidden' : 'flex min-h-0 flex-col p-4')}>
                    <div className={cn(isFull ? 'min-h-0 flex-1 overflow-y-auto px-6 py-4' : 'min-h-0 overflow-y-auto pr-1')}>
                        {children}
                    </div>
                    {footer ? (
                        <div
