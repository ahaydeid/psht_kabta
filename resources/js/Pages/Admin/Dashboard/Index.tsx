...
const komposisiAnggota = [
    ...
    { name: 'Warga', value: 0, colorClass: 'text-slate-500', dotClass: 'bg-slate-500' },
        ...
        <AdminLayout>
            ...
            <div className="space-y-4">
                <div className="mb-10 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                    {cards.map((card) => (
                        <div className={`relative min-h-35 overflow-hidden rounded-lg px-5 pb-5 pt-5 ${card.surfaceClass}`} key={card.label}>
                            <div className="absolute -right-16 -top-16 h-40 w-40 rounded-full bg-white/40" />
                            <div className="relative z-10 flex items-start justify-between gap-4">
                                <div className="flex-1 pr-12">
                                    <p className="text-xs font-semibold uppercase tracking-wide text-zinc-600">{card.label}</p>
                                    <p className="mt-1 text-4xl font-bold text-zinc-800">{card.value}</p>
                                </div>
                                <card.icon className={`absolute -right-10 -top-10 h-24 w-24 shrink-0 ${card.iconClass}`} strokeWidth={1.4} />
                            </div>
                        </div>
                    ))}
                </div>
                    ...
                    <div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
                        <div className="rounded-lg border border-zinc-200/70 bg-white p-6 lg:col-span-2">
                            <h3 className="mb-6 flex items-center justify-between text-sm font-bold text-zinc-700">
                                ...
                                <span className="text-indigo-500">{config.trendLabel}</span>
                            </h3>
                            <div className="h-64">
